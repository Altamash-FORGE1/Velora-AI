...
const Dashboard = () => {
  ...
  const tileClass = `bento-tile backdrop-blur-md border p-6 md:p-8 group cursor-pointer transition-all duration-300 ease-in-out hover:-translate-y-2 hover:shadow-2xl rounded-3xl ${
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-1">
      {/* Hero Tile */}
      <div className={`${tileClass} hero-tile md:col-span-3 lg:col-span-2`} onClick={() => navigate('/triage')}>
        <div className="flex justify-between items-start">
          <div>
            <h1 className={`text-2xl md:text-4xl font-black tracking-tight ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
            ...
            <div className="health-score-badge mt-6 bg-sky-500/10 border border-sky-400/20 px-4 py-2 rounded-full inline-flex items-center gap-2">
              ...
              <span className={`text-sm font-bold ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}>
                {isSyncing ? (
                  <span className="flex items-center gap-2">Syncing... <Loader2 size={12} className="animate-spin" /></span>
                ) : (
                  `Health Score: ${score}`
                )}
              </span>
      ...
      {/* Triage Tile */}
      <div className={`${tileClass} col-span-1`} onClick={() => navigate('/triage')}>
      ...
      {/* Locker Tile */}
      <div className={`${tileClass} col-span-1`} onClick={() => navigate('/locker')}>
        <h3 className="font-bold text-sm uppercase tracking-wider flex items-center gap-2 text-sky-600 mb-4">
      ...
      {/* BMI Tile */}
      <div className={`${tileClass} col-span-1`} onClick={() => navigate('/bmi')}>
        <h3 className="font-bold text-sm uppercase tracking-wider flex items-center gap-2 text-sky-600 mb-4">
